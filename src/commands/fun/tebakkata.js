// tebakkata - Game tebak kata dari definisi
const fs = require('fs');
const path = require('path');
const { reactProcessing } = require('../../utils/reaction');

// Game state
const games = new Map();

// Load words from JSON
let WORDS = [];
try {
    const dataPath = path.join(__dirname, '../../data/tebakkata.json');
    WORDS = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
} catch {
    // Fallback data
    WORDS = [
        { soal: 'Benda langit yang bersinar di malam hari', jawaban: 'bulan' },
        { soal: 'Hewan berkaki empat yang suka menggonggong', jawaban: 'anjing' }
    ];
}

module.exports = {
    name: 'tebakkata',
    aliases: ['kata', 'guessword'],
    description: 'Game tebak kata dari definisi',

    async execute(sock, msg, { chatId, args, senderId }) {
        try {
            const action = args[0]?.toLowerCase();

            // Start game
            if (!action || action === 'start') {
                if (games.has(chatId)) {
                    const game = games.get(chatId);
                    await sock.sendMessage(chatId, {
                        text: `⚠️ Game masih berlangsung!\n\n📝 *${game.clue}*\n\nKetik .tebakkata stop untuk berhenti.`
                    }, { quoted: msg });
                    return;
                }

                // Pick random word
                const word = WORDS[Math.floor(Math.random() * WORDS.length)];

                games.set(chatId, {
                    clue: word.soal,
                    answer: word.jawaban.toLowerCase(),
                    attempts: 0,
                    startTime: Date.now()
                });

                await sock.sendMessage(chatId, {
                    text: `*📝 TEBAK KATA*\n\n💡 *${word.soal}*\n\nKata apa yang dimaksud?\n\n⏰ Waktu: 60 detik\n💬 Ketik jawaban langsung!`
                }, { quoted: msg });

                // Auto end after 60 seconds
                setTimeout(() => {
                    if (games.has(chatId)) {
                        const game = games.get(chatId);
                        games.delete(chatId);
                        sock.sendMessage(chatId, {
                            text: `⏰ *Waktu Habis!*\n\n💡 ${game.clue}\n✅ Jawaban: *${game.answer.toUpperCase()}*`
                        });
                    }
                }, 60000);

                return;
            }

            // Stop
            if (action === 'stop' || action === 'end' || action === 'nyerah') {
                if (!games.has(chatId)) {
                    await sock.sendMessage(chatId, {
                        text: '❌ Tidak ada game yang berlangsung.'
                    }, { quoted: msg });
                    return;
                }

                const game = games.get(chatId);
                games.delete(chatId);

                await sock.sendMessage(chatId, {
                    text: `🏳️ *Menyerah!*\n\n💡 ${game.clue}\n✅ Jawaban: *${game.answer.toUpperCase()}*`
                }, { quoted: msg });
                return;
            }

            // Hint
            if (action === 'hint' || action === 'clue') {
                if (!games.has(chatId)) {
                    await sock.sendMessage(chatId, {
                        text: '❌ Tidak ada game yang berlangsung.'
                    }, { quoted: msg });
                    return;
                }

                const game = games.get(chatId);
                const hint = game.answer[0].toUpperCase() + '_'.repeat(game.answer.length - 1);
                await sock.sendMessage(chatId, {
                    text: `💡 *Hint:* ${hint} (${game.answer.length} huruf)`
                }, { quoted: msg });
                return;
            }

            // Check answer
            if (games.has(chatId)) {
                const game = games.get(chatId);
                const answer = args.join(' ').toLowerCase().trim();
                game.attempts++;

                if (answer === game.answer.toLowerCase()) {
                    const time = Math.floor((Date.now() - game.startTime) / 1000);
                    games.delete(chatId);

                    await sock.sendMessage(chatId, {
                        text: `✅ *BENAR!*\n\n💡 ${game.clue}\n✅ Jawaban: *${game.answer.toUpperCase()}*\n\n⏱️ Waktu: ${time} detik\n🔢 Percobaan: ${game.attempts}x`
                    }, { quoted: msg });
                } else if (game.attempts >= 5) {
                    games.delete(chatId);
                    await sock.sendMessage(chatId, {
                        text: `❌ *Percobaan habis!*\n\n💡 ${game.clue}\n✅ Jawaban: *${game.answer.toUpperCase()}*`
                    }, { quoted: msg });
                } else {
                    await sock.sendMessage(chatId, {
                        text: `❌ Salah! Sisa percobaan: ${5 - game.attempts}x\n\n💡 Ketik .tebakkata hint untuk petunjuk.`
                    }, { quoted: msg });
                }
            }

        } catch (err) {
            await sock.sendMessage(chatId, {
                text: '❌ Error: ' + err.message
            }, { quoted: msg });
        }
    }
};
